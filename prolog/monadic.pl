...
% (t -> Mt) -> alpha => alpha
lower(cat(tower(B,s,s),K,to(to(t,m(t)),Alpha)),
      cat(B,app(K,lam(X,dys(X,[],[]))),Alpha)).
...
% (Mt -> Mt) -> Mt => Mt
lower(cat(tower(s,s,s),K,to(to(m(t),m(t)),m(t))),
      cat(s,app(K,lam(X,X)),m(t))).


% single-level tower(s,s,C), with C = s\np or s/np
% apply continuation over X and abstract again over X
% ((e -> t) -> Mt) -> Mt => e -> Mt
lower(cat(tower(s,s,C),K,to(to(to(e,t),m(t)),m(t))),
      cat(C,lam(X,app(K,lam(P,dys(app(P,X),[],[])))),to(e,m(t))))
:-
	( C = bslash(s,Y) ; C = fslash(s,Y) ),
	atom(Y).
...
lower(cat(tower(s,s,A),Sem,to(to(T,m(t)),m(t))),cat(B,Lowered,m(t))) :-
...
% if A:a => C:c
lower(cat(tower(s,s,A),Sem,to(to(Beta,Alpha),Alpha)),
      cat(B,Lowered,to(Gamma,Alpha))) :-
        A = tower(tower(_,_,_),_,_),
