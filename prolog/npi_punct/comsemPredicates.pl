/*************************************************************************
    ...
    Modifications copyright (C) 2017, Michael White
...
% mww: changed to use triples (Reading,Num,[Equivs])
printRepresentations(Readings):-
   printRep(Readings).

printRep([]):- nl.
printRep([(Reading,Num,Equivs)|OtherReadings]):-
   atomics_to_string([Num|Equivs],', ',Str),
   nl, write('Reading '), write(Str), nl, nl,
   \+ \+ (numbervars(Reading,0,_), print(Reading)),
   nl,
   % mww: add FOL translation for complete DySs
   (   Reading \= dys(_,_,_) -> nl
   ;   dys2fol(Reading,Formula),
       \+ \+ (numbervars(Formula,0,_), print(Formula)),
       nl, nl ),
   printRep(OtherReadings).
...
% mww: added dys etc.; also changed implication from '>' to '->'
user:portray(dys(E,DRs,Conds)) :- bbmode(infix), !,
	write('{ <'), print(E),
	(   DRs=[]
	->  write(', _')
	;   write(', +'), write_term(DRs,[numbervars(true)]) ),
	(   Conds=[]
	-> write('> }')
	;  write('> | '), printConds(Conds), write(' }') ).

% mww: added all/3, most/3, few/3
user:portray(all(X,R,S)):- bbmode(infix), !,
	write('(all '), write_term(X,[numbervars(true)]),
	write(' '), print(R), write(' '), print(S), write(')').
user:portray(most(X,R,S)):- bbmode(infix), !,
	write('(most '), write_term(X,[numbervars(true)]),
	write(' '), print(R), write(' '), print(S), write(')').
user:portray(few(X,R,S)):- bbmode(infix), !,
	write('(few '), write_term(X,[numbervars(true)]),
	write(' '), print(R), write(' '), print(S), write(')').
...
user:portray(some(X,F)):- bbmode(infix), !, write('(some '), write_term(X,[numbervars(true)]), tab(1), print(F), write(')').
user:portray(all(X,F)):- bbmode(infix), !, write('(all '), write_term(X,[numbervars(true)]), tab(1), print(F), write(')').
user:portray(lam(X,F)):- bbmode(infix), !, write('(lam '), write_term(X,[numbervars(true)]), tab(1), print(F), write(')').
...
printConds([Cond]) :- !, print(Cond).
printConds([Cond|Conds]) :- print(Cond), write(' & '), printConds(Conds).
